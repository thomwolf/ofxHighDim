...
void TSNE::run(double* X, int N, int D, double* Y, int *fixedPointsIndexes, int no_dims, double perplexity, double theta, int max_iter, bool runManually){

    this->X = X;
    this->N = N;
    this->D = D;
    this->Y = Y;
    this->fixedPointsIndexes = fixedPointsIndexes;
...
//    omp_set_num_threads(num_threads);
    ...
    uY    = (double*) calloc(N * no_dims , sizeof(double));
...
void TSNE::computeGaussianPerplexity(double* X, int N, int D, int** _row_P, int** _col_P, double** _val_P, double perplexity, int K) {
    ...
    *_val_P = (double*) calloc(N * K, sizeof(double));
    ...
    // row_P - offset for col_P,
    // col_P recorded in the index K nearest neighbors
    // val_P recorded in K numbers, apparently p {i | j}
...
//    #pragma omp parallel for
    for (int n = 0; n < N; n++)
    {
        ...
        while (!found && iter < 200) {
            ...
            // in the latest version of TSNE : square distances....
...
void TSNE::runIteration(int N_fixed_points) {
...
//                Y[fixed_points_indices[i] * no_dims + d] = fixed_points_coordinates[i * no_dims + d];
//            }
//        }
//    }
    this->N_fixed_points = N_fixed_points;
    // Compute approximate gradient
    computeGradient(row_P, col_P, val_P, Y, N, no_dims, dY, theta);
    
    int current_fixed_point = 0; //fixed_points_indices is an ordered list of the fixed points indices
    ...
    for (int i = 0; i < N * no_dims; i++) {
        ...
        // Perform gradient update (with momentum and gains) if the point is not fixed
        if (N_fixed_points > 0 && current_fixed_point < N_fixed_points){
            if (i == fixedPointsIndexes[current_fixed_point]){
                uY[i] = 0;
                current_fixed_point++;
            } else {
...
void TSNE::zeroMean(double* X, int N, int D) {
    ...
    for (int n = 0; n < N; n++) {
        // Perform gradient update (with momentum and gains) if the point is not fixed
        if (N_fixed_points > 0 && current_fixed_point < N_fixed_points){
            if (n == fixedPointsIndexes[current_fixed_point]){
                current_fixed_point++;
            } else {
                for (int d = 0; d < D; d++) {
                    mean[d] += X[n * D + d];
    ...
    for (int d = 0; d < D; d++) {
        mean[d] /= ((double) N - (double) N_fixed_points);
    }

    current_fixed_point = 0; //fixed_points_indices is an ordered list of the fixed points indices
    // Subtract data mean
    for (int n = 0; n < N; n++) {
        if (N_fixed_points > 0 && current_fixed_point < N_fixed_points){
            if (n == fixedPointsIndexes[current_fixed_point]){
