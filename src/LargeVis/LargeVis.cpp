...
void LargeVis::test_accuracy()
{
	...
	{
		x = floor(distribution(generator) * (n_vertices - 0.1));
...
void LargeVis::visualize_thread(int id)
{
	...
	while (1)
	{
		...
		p = sample_an_edge(distribution(generator), distribution(generator));
		x = edge_from[p];
		y = edge_to[p];
		lx = x * out_dim;
		...
		for (i = 0; i < n_negatives + 1; ++i)
		{
			if (i > 0)
			{
				y = neg_table[(unsigned long long)floor(distribution(generator) * (neg_size - 0.1))];
...
void LargeVis::visualize()
{
	...
	for (i = 0; i < n_vertices * out_dim; ++i) vis[i] = (distribution(generator) - 0.5) / out_dim * 0.0001;
	init_neg_table();
	init_alias_table();
	edge_count_actual = 0;
	pthread_t *pt = new pthread_t[n_threads];
...
//	gsl_rng_set(gsl_r, 314159265);
    std::uniform_real_distribution<double>::param_type d(0, 1);
    distribution.param(d);
    ...
    for (i = 0; i < n_vertices * out_dim; ++i)
        vis[i] = (distribution(generator) - 0.5) / out_dim * 0.0001;
    init_neg_table();
    init_alias_table();
    edge_count_actual = 0;
    
    long long edge_count = 0, last_edge_count = 0;
    long long x, y, p, lx, ly, j;
    real f, g, gg, cur_alpha = initial_alpha;
    real *cur = new real[out_dim];
    real *err = new real[out_dim];
    real grad_clip = 5.0;
